...
# Find and kill any existing Streamlit processes to ensure a clean restart
# This is to prevent issues where an old app.py might still be running
cmd = "ps aux | grep 'streamlit run' | grep -v grep | awk '{print $2}'"
process_ids = subprocess.getoutput(cmd).splitlines()
for pid in process_ids:
    try:
        os.kill(int(pid), signal.SIGTERM)
        print(f"Killed existing Streamlit process with PID: {pid}")
    except OSError as e:
...
# Task
Install the required libraries (`streamlit`, `pandas`, `numpy`, `scikit-learn`, `joblib`) in the environment and then restart the Streamlit application `app.py`.

## Remove `pip install` from application code

### Subtask:
Remove the `pip install` commands from the `app.py` file.

**Reasoning**:
I need to read the content of `app.py`, remove the `pip install` lines, and then write the modified content back to `app.py`. First, I will read the file.
"""

with open('app.py', 'r') as f:
    app_content = f.readlines()

# Filter out pip install lines
filtered_content = [
    line for line in app_content
...
print("Removed 'pip install' commands from app.py.")

"""## Ensure libraries are installed in the environment

### Subtask:
Install all necessary libraries (streamlit, pandas, numpy, scikit-learn, joblib) in the current environment.

**Reasoning**:
To ensure all dependencies for the Streamlit application are met, I will install the required libraries using pip.
"""
...
required_libraries = ['streamlit', 'pandas', 'numpy', 'scikit-learn', 'joblib']
...
### Subtask:
Restart the Streamlit application (`app.py`) to reflect the changes and ensure it runs correctly with all dependencies installed.

## Summary:

### Data Analysis Key Findings
*   The `app.py` file was successfully modified to remove all `pip install` commands, ensuring that dependencies are managed by the environment rather than the application code itself.
*   All necessary libraries for the Streamlit application have been confirmed as installed in the environment. Specifically, `streamlit`, `pandas`, `numpy`, and `joblib` were already present, while `scikit-learn` was identified as missing and subsequently installed.

### Insights or Next Steps
*   With the `pip install` commands removed from `app.py` and all dependencies installed, the next logical step is to proceed with restarting the Streamlit application (`app.py`) as per the overarching task.

# Task
Regenerate the `app.py` file without any `IPython.display` import, then confirm all necessary Python libraries are installed, and finally kill any running Streamlit processes before restarting the `app.py` application.

## Verify and Clean `app.py`

### Subtask:
Double-check the contents of `app.py` to ensure the line `from IPython.display import display` (or any similar Jupyter-specific import) is completely removed. I will regenerate the file content to be certain.

**Reasoning**:
To ensure that no Jupyter-specific imports are present and to explicitly regenerate the `app.py` file, I will provide the complete Streamlit application code again, using `%%writefile app.py`.
"""
...
### Data Analysis Key Findings
*   The Streamlit application code for `app.py` was explicitly regenerated using `%%writefile`, ensuring that any `IPython.display` import statements were removed.
*   This step confirms that the `app.py` file is clean of Jupyter-specific imports and ready for deployment in a Streamlit environment.

### Insights or Next Steps
*   The `app.py` file is now verified and cleaned, fulfilling the requirements of this subtask. The next steps will involve confirming library installations and restarting the Streamlit application.

## Ensure Dependencies are Installed

### Subtask:
Confirm that all necessary Python libraries (streamlit, pandas, numpy, scikit-learn, joblib) are correctly installed in the environment. This step will re-run the installation check.

**Reasoning**:
To confirm that all necessary Python libraries are installed, I will execute a Python script that checks for the presence of each library and installs it if missing.
"""
...
### Data Analysis Key Findings
*   The `app.py` file was explicitly regenerated, ensuring all `IPython.display` imports were removed and the Streamlit application code is clean.
*   All necessary Python libraries (`streamlit`, `pandas`, ``numpy`, `scikit-learn`, `joblib`) have been confirmed as installed in the environment. `scikit-learn` was successfully installed during this step, while others were already present.

### Insights or Next Steps
*   With `app.py` validated and all dependencies met, the environment is now fully prepared to restart the Streamlit application. The next step is to proceed with restarting the Streamlit application (`app.py`) to confirm its proper functioning.

**Reasoning**:
The subtask of ensuring all necessary Python libraries are installed has been completed. The next logical step, as per the overarching task, is to kill any running Streamlit processes before restarting the `app.py` application.
"""
...
### Data Analysis Key Findings
*   The `app.py` file was successfully regenerated, confirming the removal of any `IPython.display` imports, thus ensuring it is clean for a pure Streamlit environment.
*   All required Python libraries (`streamlit`, `pandas`, `numpy`, `scikit-learn`, `joblib`) were confirmed to be installed in the environment.
*   Any previously running Streamlit processes were successfully terminated, and the `app.py` application was restarted, making it ready for interaction.

### Insights or Next Steps
*   The Streamlit application is now fully prepared and running. Users can interact with it using the provided URL to predict flight delays based on various inputs. This concludes the entire task of summarizing, setting up, and demonstrating the Streamlit application.
...
## Kill and Restart Streamlit Application

### Subtask:
Explicitly kill any previously running Streamlit processes to prevent conflicts or old versions from being served. Then, restart the Streamlit application, making sure to specifically launch `app.py`.

## Summary:

### Data Analysis Key Findings

*   The `app.py` file was successfully regenerated, confirming the removal of any `IPython.display` imports, ensuring it is clean for a pure Streamlit environment.
*   All required Python libraries including `streamlit`, `pandas`, `numpy`, `scikit-learn`, and `joblib` are now confirmed installed; `scikit-learn` was explicitly installed during this process.
*   Any previously running Streamlit processes (e.g., with PID 8733, as noted in the log) were terminated, and the `app.py` application was successfully restarted.

### Insights or Next Steps

*   The Streamlit application is now fully prepared and running, ready for user interaction to predict flight delays based on various inputs.
*   The environment is cleanly configured, ensuring the application runs without conflicts from previous instances or missing dependencies.
"""